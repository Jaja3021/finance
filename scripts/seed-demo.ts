...
async function main() {
  const old = await db.select().from(schema.users).where(eq(schema.users.email, EMAIL)).get();
  if (old) await db.delete(schema.users).where(eq(schema.users.id, old.id)).run();
  ...
  const cats = new Map((await db.select().from(schema.categories).where(eq(schema.categories.userId, user.id)).all()).map((c) => [c.name, c.id]));

  const acct = async (name: string, type: AccountType, currency: string, opening: number, institution: string, color: string, extra = {}) =>
    await db.insert(schema.accounts).values({ ownerId: user.id, name, type, currency, openingBalance: opening, institution, color, ...extra }).returning().get();
  const gcash = await acct("GCash", "ewallet", "PHP", 350000, "gcash", "#0070e0");
  const bdo = await acct("BDO Savings", "bank", "PHP", 8500000, "bdo", "#1a3d8f");
  const usd = await acct("BDO Dollar Savings", "bank", "USD", 120000, "bdo-usd", "#1a3d8f");
  const card = await acct("BPI Credit Card", "credit_card", "PHP", -1850000, "bpi-cc", "#b11116", { creditLimit: 10000000, interestRate: 36 });
  const cash = await acct("Cash Wallet", "cash", "PHP", 250000, "cash", "#4b5563");
  ...
  await db.insert(schema.transactions).values(txs).run();
